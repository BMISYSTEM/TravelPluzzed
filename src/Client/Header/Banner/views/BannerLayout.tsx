...
export const BannerLayout = () => {
  ...
  if (buscador !== "") {
    const buscarCiudad = ciudadDetail?.filter((data) =>
      data.nombre.toLowerCase().includes(buscador.toLowerCase()),
    );
    const buscarPaises = paises?.filter((data) =>
      data.nombre.toLowerCase().includes(buscador.toLowerCase()),
    );
    if (buscarCiudad?.length > 0) {
      paises = paises?.filter(
        (data) => Number(data.id) === Number(buscarCiudad?.[0]?.pais),
      );
      console.log(paises);
    }
    if (buscarPaises?.length !== 0) {
      ciudadDetail = ciudadDetail.filter(
        (data) => Number(data.pais) === Number(buscarPaises?.[0]?.id),
      );
      paises = paises?.filter(
        (data) => Number(data.id) === Number(buscarPaises?.[0]?.id),
      );
    }
  }
  return (
    <section
      className=" relative w-full h-full bg-slate-700   "
      style={{
        height: "85vh",
      }}
    >
      <div className="  absolute w-full h-full  flex flex-col gap-3 justify-center items-center p-3">
        <div className="relative md:w-1/2 w-full h-full flex flex-col gap-3  p-3 justify-end z-[25]  ">
          <h1
          ...
          <form action="">
            <input
              ...
              placeholder="Busca tu destino con nosotros"
              className="border-2 border-[rgb(243,245,79)] p-3 rounded-xl md:w-[40rem] w-full"
              onClick={() => setIslandSheart(true)}
            />
            {islandSheart ? (
                ...
                <div className="w-full flex flex-wrap p-3 gap-x-20 gap-y-10 justify-center h-96  ">
                  {pais?.succes
                    ? paises?.map((pais, index) => (
                        <LinkBuscador
                          key={index}
                          textPrimary={pais?.nombre}
                          textSecond={ciudadDetail?.filter(
                            (data) => Number(data.pais) === Number(pais.id),
                          )}
                          id={pais?.id}
                        />
                      ))
...
const CarrouselImagen = () => {
  ...
  return (
    ...
    >
      <div className="absolute w-full h-full bg-black/20 z-[20] flex flex-row items-start p-3 justify-end">
        {visibleImagen === 1 ? (
      ...
      {visibleImagen === 1 ? (
        <img
          src={travel1}
          alt="Imagen de travel"
          className="animate__animated animate__fadeIn w-full h-full md:object-fill object-cover"
        />
      ) : null}
      {visibleImagen === 2 ? (
        <img
